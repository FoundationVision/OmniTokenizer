...
def main(args):
    ...
    if args.vae_type == "sd1.4":
        vae = AutoencoderKL.from_pretrained(args.pretrained_model_path, subfolder="vae").to(device)
        # Freeze vae and text_encoder
        vae.requires_grad_(False)

    else:
        vae = OmniTokenizer_VQGAN.load_from_checkpoint(args.pretrained_model_path).to(device)
        vae.use_vae = True
        vae.codebook._need_init = False
    ...
    for _ in pbar:
        # Sample inputs:
        latent_dim = 4 if args.vae_type == "sd1.4" else 8
        num_frames = args.num_frames if args.vae_type == "sd1.4" else 1 + (args.num_frames - 1) // 4
        if args.use_fp16:
            z = torch.randn(n, num_frames, latent_dim, latent_size, latent_size, dtype=torch.float16, device=device)
        else:
            z = torch.randn(n, num_frames, latent_dim, latent_size, latent_size, device=device)
        ...
        if args.vae_type == "sd1.4":
            ...
            # b t h w c
            samples = rearrange(samples, "b f c h w -> b f h w c")
            samples = vae.decode(samples / 0.18215, is_image=False)
            samples = rearrange(samples, "b c f h w ->  b f c h w")
            for i, sample in enumerate(samples):
                # sample = ((sample + 0.5) * 255).add_(0.5).clamp_(0, 255).to(dtype=torch.uint8).cpu().permute(0, 2, 3, 1).contiguous()
                sample = (torch.clamp(sample + 0.5, 0, 1) * 255).to(dtype=torch.uint8).cpu().permute(0, 2, 3, 1).contiguous()
                index = i * dist.get_world_size() + rank + total
                # Image.fromarray(sample).save(f"{sample_folder_dir}/{index:04d}.png")
                sample_save_path = f"{sample_folder_dir}/{index:04d}.mp4"
                imageio.mimwrite(sample_save_path, sample, fps=8, quality=9)
