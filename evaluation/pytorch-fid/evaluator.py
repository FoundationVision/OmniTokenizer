...
def create_npz_from_sample_folder(sample_dir, num=-1):
    """
    Builds a single .npz file from a folder of .png samples.
    """
    imgs = os.listdir(sample_dir)
    if "." not in imgs[0]:
        imgs = glob.glob(f"{sample_dir}/*/*.JPEG")
...
def main():
    ...
    if os.path.isdir(args.ref_batch):
        args.ref_batch = create_npz_from_sample_folder(args.ref_batch)
    else:
        assert args.ref_batch.endswith(".npz")
    ...
    if os.path.isdir(args.sample_batch):
        args.sample_batch = create_npz_from_sample_folder(args.sample_batch)
    else:
        assert args.sample_batch.endswith(".npz")
